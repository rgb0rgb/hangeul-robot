...
> **처음 쓰는 분은 저장소 맨 위의 `Setup.bat` 을 두 번 누르세요.** 파이썬 설치, 부품 설치,
> 바탕화면 바로가기까지 만들고 시작합니다. 문제가 생기면 `Reset.bat`(바탕화면 [한글 로봇 초기화]).
> 장치는 COM 번호가 아니라 USB 신원으로 찾으므로 아래 표의 장치 이름을 고칠 필요가 없습니다.
> 아래는 WSL 에서 돌리는 사람을 위한 기록이다.
